...
def correlation_between_labels(df):
    # Only select the disease columns, excluding the filename column
    disease_columns = ['opacity', 'diabetic retinopathy', 'glaucoma', 'macular edema', 
                      'macular degeneration', 'retinal vascular occlusion', 'normal']
    
    plt.figure(figsize=(15, 8))
    plt.title("Correlation between the different categories")
    sns.heatmap(df[disease_columns].corr(), cmap='YlGnBu', annot=True)
    plt.show()
    return
